...
export const PrivateVoi: FC = () => {
	...
	const getData = async () => {
		...
		try {
			...
			const unarchivedData=data.listData.filter((val:any)=>{
				if( val.status=="unarchived" || val.status == undefined || val.status == null ){
					return val
				}else{}
			})
	...
	<React.Fragment>
		...
		<Paper className={classes.root}>
			{
				userLoginType === 'Admin' || (voiPrivateAccessSites && voiPrivateAccessSites.length > 0 && voiPrivateAccessSites[0]) ?
				...
				<Table stickyHeader aria-label="sticky table">
					<TableHead>
						<TableRow>
							...
							{
								userLoginType === 'Admin' || (allVoiSites && allVoiSites.length > 0 && allVoiSites[0] && voiPrivateAccessSites && voiPrivateAccessSites.length > 0 && voiPrivateAccessSites[0]) ? <TableCell>Action</TableCell> : ""
							}
						</TableRow>
					</TableHead>
					<TableBody>
							...
							voiLists && voiLists.listData && voiLists.listData.map((eachList:any) => {
								...
								return (
									<TableRow>
										...
										<TableCell>
											<Tooltip title={eachList.vrms.length > 0 ? (eachList.vrms).join(',\n') : "NA"} arrow placement="top">
											<p>{eachList.vrms.length > 0 ? `${eachList.vrms.length} VRMs` : "0 VRM"}</p>
											</Tooltip>
										</TableCell>
										{/* <TableCell>{eachList.vrms && (eachList.vrms).length > 0 ? (eachList.vrms).join(',\n') : 'NA'}</TableCell> */}
										{
											userLoginType === 'Admin' || (allVoiSites && allVoiSites.length > 0 && allVoiSites[0] && voiPrivateAccessSites && voiPrivateAccessSites.length > 0 && voiPrivateAccessSites[0]) ? 
												<TableCell>{ (userLoginType === 'Admin' || (voiPrivateAccessSites && voiPrivateAccessSites.length > 0 && voiPrivateAccessSites[0] && voiPrivateAccessSites.includes(eachList.site))) ? getActions(eachList) : "NA" }</TableCell> 
											: ""
										}
									</TableRow>
								)
    	...
    	<AddVOI addOpen={addVOIOpen} closeDialog={handleToggleAdd} refreshData={getData} emailGroups={emailGroups} voiType="private"/>
    	<EditVOI editOpen={editOpen} closeDialog={handleCloseEdit} editData={selectedVOIList} refreshData={getData} emailGroups={emailGroups} voiType="private"/>
    	<DeleteVOI deleteOpen={deleteOpen} closeDialog={handleCloseDelete} deleteData={selectedVOIList} refreshData={getData} voiType="private"/>
