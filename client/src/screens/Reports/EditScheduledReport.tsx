...
import { EditScheduledReportSchema } from "../../validationScheemas/ScheduledReportSchema";
...
export const EditScheduledReport: FC<Props> = ({editOpen, closeDialog, editData, refreshData}) => {
	...
	const editScheduleSites = editData.site ? [editData.site] : [];
	const editScheduleFrequency = editData.scheduleFrequency ? [editData.scheduleFrequency] : [];
	...
	const handleSubmit = async (values: FormikValues) => {
		try {
			...
			await axios.put("/api/scheduledReports", values, {
				headers: { authorization: "Bearer " + localStorage.getItem("token") }
			});

			enqueueSnackbar("Report schedule updated successfully.");
			cancelEdit();
			refreshData();

		} catch (e:any) {
			console.log("Error in edit report schedule:",e);
	...
	const formik = useFormik({
		...
		validationSchema: EditScheduledReportSchema,
	...
	useEffect(() => {
		formik.setValues({
			...
			scheduleFrequency: editData.scheduleFrequency ? editData.scheduleFrequency : "",
			site: editData.site ? editData.site : "",
			insightsData: editData.insightsData ? String(editData.insightsData) : "false",
            emailTemplateHeader: "",
            emailTemplateFooter: "",
        });
	...
	useEffect(() => {
		const setInitialSites = () => {
			setSelectedSites(editScheduleSites);
			setSelectedScheduleFrequency(editScheduleFrequency);
		}
		setInitialSites();
		// eslint-disable-next-line
	}, [editData]);
							...
							<div className="--margin-bottom-large">
								...
								<div style={{marginLeft:"0", minWidth: isMobile ? "260px" : "285px", marginTop:"15px"}} role="group" onChange={formik.handleChange} >
									<label style={{cursor:"pointer"}}>
										<input type="radio" checked={formik.values.insightsData !== 'true'} name="insightsData" value="false" id="false" style={{marginRight:"5px"}} />
										FALSE 
									</label>
									<label style={{cursor:"pointer"}}>
										<input style={{marginLeft:"50px", marginRight:"5px"}} checked={formik.values.insightsData === 'true'} type="radio" name="insightsData" value="true" id="true" />
										TRUE 
									</label>
								</div>
							</div>
